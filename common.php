...
if (!function_exists('makeStr')) {
     ...
     *  生成字段
     */
    function makeStr($type = 'salt', $length = '6', $option = false)
    {
        $str = "";
        $salt = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';   #字符串可用下标的方式取值
        do {
            $str = substr(str_shuffle($salt), 0, $length);
            if ($option == true) return $str;
        } while (db('users')->where([$type => $str])->find());
        return $str;
    }
}
...
if (!function_exists('makePassword')) {
     ...
     *  生成密碼
     */
    function makePassword($salt, $password)
    {
...
if (!function_exists('dm')) {
    function dm($var)
    {
        dump(json_decode(json_encode($var, true), true));
        die;
    }
}
/**
